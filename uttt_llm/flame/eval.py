...
import wandb
# W&B authentication comes from the WANDB_API_KEY environment variable.
...
def main(job_config: JobConfig):
    ...
    #########################################################################
    # handle the case where the model has buffers that are not initialized on the correct device
    # call the reset_parameters() for the model
    ...
    # load the checkpoitn 
    # dcp.load(state_dict=checkpoint_path)
    logger.warning(f"Loading checkpoint from {job_config.checkpoint.initial_load_path}")
    DCP.load(state_dict={"model": model.state_dict()}, checkpoint_id=job_config.checkpoint.initial_load_path)
    # DCP.load(state_dict=model.state_dict(), checkpoint_id=job_config.checkpoint.initial_load_path)
    ...
    with (
        ...
        while train_state.step < job_config.training.steps:
            ...
            # per_token_loss = F.cross_entropy(flatten_logits, flatten_labels, reduction="none") # [bs * seq_len]
            criterion = FusedCrossEntropyLoss(reduction="none")

            per_token_loss = criterion(flatten_logits, flatten_labels)
            ...
            logger.info(
                f"Progress: {train_state.step:5d} / {job_config.training.steps:5d} "
        ...
        # save to json
        if torch.distributed.get_rank() == 0:
            loss_tracker.save_to_json(os.path.join(job_config.job.dump_folder, "loss_tracker.json"))
            loss_at_per_token_tracker.save_to_json(os.path.join(job_config.job.dump_folder, "loss_at_per_token_tracker.json"))
