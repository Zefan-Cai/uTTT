...
from flame.checkpoint_manager import CheckpointManager
...
def _wandb_login_from_env() -> None:
    disabled = os.environ.get("DISABLE_WANDB", "").strip().lower()
    if disabled in {"1", "true", "yes", "y", "on"}:
        return

    base_url = (
        os.environ.get("WANDB_BASE_URL")
        or os.environ.get("WANDB_HOST")
        or "https://api.wandb.ai"
    ).rstrip("/")
    os.environ["WANDB_BASE_URL"] = base_url
    os.environ["WANDB_HOST"] = base_url
    ...
    except Exception as exc:  # pragma: no cover - W&B should not crash training bootstrap.
        logger.warning("W&B login from environment failed: %s", exc)


_wandb_login_from_env()
...
def _metric_to_scalar_tensor(value, device):
    """Convert a model-output metric to a detached scalar tensor."""
    if value is None:
        return None
...
def _append_model_output_metrics(output, metric_accums, device):
    """Accumulate loss and MoE statistics emitted by a model forward pass."""
    metric_sources = {
        "loss/total": getattr(output, "loss", None),
        "loss/ce": getattr(output, "ce_loss", None),
        "loss/lb_scaled": getattr(output, "lb_loss", None),
    }
    moe_stats = getattr(output, "moe_stats", None)
    if not isinstance(moe_stats, dict):
        moe_stats = {}
    stat_name_map = {
        "apply_lb_loss": "loss/lb_apply_unscaled",
        "update_lb_loss": "loss/lb_update_unscaled",
        "lb_loss": "loss/lb_scaled",
        "num_experts": "moe/num_experts",
    }
    for stat_name, stat_value in moe_stats.items():
        metric_name = stat_name_map.get(stat_name, f"moe/{stat_name}")
        # Prefer the explicit model-output field when both sources expose the
        # same metric name. This keeps the logging contract deterministic.
        if (
            metric_name not in metric_sources
            or metric_sources[metric_name] is None
        ):
            metric_sources[metric_name] = stat_value

    for metric_name, metric_value in metric_sources.items():
        scalar = _metric_to_scalar_tensor(metric_value, device)
        if scalar is not None:
            metric_accums.setdefault(metric_name, []).append(scalar)


def _finalize_model_output_metrics(metric_accums, parallel_dims, world_mesh):
    """Average accumulated metrics locally and across the DP/CP mesh."""
    metrics = {}
    for metric_name, values in metric_accums.items():
        if not values:
            continue
        ...
        if (
            ...
            value = dist_utils.dist_mean(value, world_mesh["dp_cp"])
        metrics[metric_name] = (
...
def main(job_config: JobConfig):
    ...
    # All ranks must participate in DTensor.full_tensor() collectives; only rank 0 logs.
    # full_tensor() materializes a temporary replicated Tensor and leaves the parameter sharded.
    rank = torch.distributed.get_rank()
    for name, param in model.named_parameters():
        with torch.no_grad():
            p = param.detach()
            local_shape = list(p.to_local().shape) if hasattr(p, "to_local") else list(p.shape)
            p_full = p.full_tensor() if hasattr(p, "full_tensor") else p

            if rank == 0:
                if p_full.is_floating_point() and p_full.numel() > 0:
                    p_float = p_full.float()
                    values = ""
                    if p_full.numel() <= 128:
                        values = f", values={p_float.cpu().tolist()}"
                    logger.info(
                        f"[param-check] {name}: shape={list(p_full.shape)}, local_shape={local_shape}, "
                        f"mean={p_float.mean().item():.6f}, std={p_float.std(unbiased=False).item():.6f}"
                        f"{values}"
                    )
                    del p_float
                else:
    ...
    checkpoint_loaded = checkpoint.load(
        step=job_config.checkpoint.load_step
    )
    if os.environ.get("TRAIN_ATTEMPT_KIND") == "resume":
        if not checkpoint_loaded:
            raise RuntimeError(
                "Resume attempt requires a successfully loaded checkpoint"
            )
        if int(train_state.step) < 1:
            raise RuntimeError(
                "Resume attempt loaded an invalid zero-step checkpoint"
            )
        logger.info(
            "Verified resume checkpoint at train step %s",
            train_state.step,
        )
    ...
    # save the wandb id to the file in dump_folder
    # check if wandb is enabled
    if job_config.metrics.enable_wandb:
        if torch.distributed.get_rank() == 0:
            wandb_run = getattr(getattr(metric_logger.logger, "wandb", None), "run", None)
            wandb_run_id = getattr(wandb_run, "id", None)
            if wandb_run_id:
                wandb_file = os.path.join(job_config.job.dump_folder, "wandb_id.txt")
                if not os.path.exists(wandb_file):
                    with open(wandb_file, "w") as f:
                        f.write(wandb_run_id)
            else:
                logger.warning("WandB logger unavailable; skipping wandb_id.txt write.")
    ...
    # The GPU count enters through the launcher, so a config alone cannot pin
    # the effective batch. The published experiments are defined at a fixed
    # global batch (32 sequences for Table 1); when the exp config declares it,
    # hold the run to that rather than training a silently different experiment.
    expected_global_batch_size = int(
        getattr(job_config.training, "expected_global_batch_size", 0) or 0
    )
    if expected_global_batch_size and global_batch_size != expected_global_batch_size:
        raise ValueError(
            f"Effective global batch size is {global_batch_size} "
            f"({job_config.training.batch_size} per device x {dp_degree} data-parallel "
            f"x {job_config.training.gradient_accumulation_steps} grad-accum steps), "
            f"but this experiment is defined at {expected_global_batch_size}. "
            f"Scale training.batch_size or the GPU count so the product matches, or "
            f"remove training.expected_global_batch_size to disable this check."
        )
    # train loop
    ...
    with (
        ...
        while train_state.step < job_config.training.steps:
            train_state.step += 1
            os.environ["FLAME_TRAIN_STEP"] = str(train_state.step)
            ...
            # PyTorch foreach norm can hit CUDA illegal memory access on Gemma4's
            # large bf16 gradients. The scalar norm path is slower but stable.
            grad_clip_foreach = not any(
                getattr(getattr(m, "config", None), "model_type", None)
                in {"gemma4_text", "custom_gemma4", "custom_recurrent_gemma4"}
                for m in model_parts
            )
            ...
            if metric_logger.should_log(train_state.step):
                ...
                extra_metrics.update(
                    _finalize_model_output_metrics(
                        model_metric_accums,
                        parallel_dims,
                        world_mesh,
                    )
