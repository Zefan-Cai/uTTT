...
class JobConfig:
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            "--optimizer.alpha_weight_decay",
            dest="optimizer.alpha_weight_decay",
            action="store_true",
            default=False,
            help="""
            Whether to apply optimizer weight decay to learnable alpha blend parameters
            such as prev_chunk_alpha(s) and first_chunk_alpha(s). This is disabled
            by default.
            """,
        )
        self.parser.add_argument(
            "--optimizer.no_alpha_weight_decay",
            dest="optimizer.alpha_weight_decay",
            action="store_false",
            help="Exclude learnable alpha blend parameters from optimizer weight decay.",
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            ...
            dest="checkpoint.model_weights_only",
            action="store_true",
            default=False,
            help="""
                Save only model weights in periodic checkpoints. This omits optimizer,
                lr scheduler, dataloader, and train state, so the checkpoint is suitable
                for evaluation/export but not for exact training resume.
            """,
        )
        self.parser.add_argument(
            "--checkpoint.no_model_weights_only",
            dest="checkpoint.model_weights_only",
            action="store_false",
            help="Save full training checkpoints.",
        )
        self.parser.add_argument(
            "--checkpoint.save_backend",
            type=str,
            default="dcp",
            choices=["dcp", "rank0_model_only"],
            help="""
                Checkpoint save backend. "dcp" keeps the standard TorchTitan
                distributed checkpoint path. "rank0_model_only" bypasses
                torch.distributed.checkpoint save and writes a rank-0
                model.pt export for evaluation; it requires
                --checkpoint.model_weights_only and is not suitable for exact
    ...
    def parse_args(self, args_list: list = sys.argv[1:]):
        ...
        if "--optimizer.no_alpha_weight_decay" in args_list:
            args_dict["optimizer"]["alpha_weight_decay"] = False
        if "--checkpoint.no_model_weights_only" in args_list:
            args_dict["checkpoint"]["model_weights_only"] = False
