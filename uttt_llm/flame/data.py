...
class BufferShuffledIterableDataset(IterableDataset):
    ...
    def tokenize(self, data, batch_size: int = 64):
        texts, states = [], []
        for sample in data:
            # Pre-tokenized rows (e.g. the single-document evaluation set) carry
            # their ids directly. Yielding them untouched keeps each row's exact
            # token count, which is what makes the packer cut on document
            # boundaries instead of mid-document.
...
class OnlineTokenizedIterableDataset(IterableDataset):
    ...
    def tokenize(self, data, buffer_size: int = 64):
        buffer, states = [], []
        for sample in data:
            # Pre-tokenized rows (the single-document evaluation set) already
            # carry their ids. Passing them through unchanged preserves each
            # row's exact token count, which is what makes the packer cut on
            # document boundaries rather than mid-document.
            if sample.get('input_ids', None) is not None:
                if buffer:
