...
def grouped_gemm_transpose_triton(
    ...
    def grid(meta):
        BN = meta["BLOCK_N"]
        if debug_autotune:
            # Print the final tuned configuration used for this launch
            print(
                "[triton.autotune] selected config: "
                f"BLOCK_M={meta.get('BLOCK_M')}, "
                f"BLOCK_N={meta.get('BLOCK_N')}, "
                f"BLOCK_K={meta.get('BLOCK_K')}, "
                f"num_warps={meta.get('num_warps')}, "
                f"num_stages={meta.get('num_stages')}"
            )
...
def benchmark_gemm_fwd():
    from benchmark import run_benchmark_fwd_only
    import argparse
    import os

    parser = argparse.ArgumentParser()
    parser.add_argument("--save_dir", type=str, default="grouped_gemm_benchmark")
    args = parser.parse_args()
    os.makedirs(args.save_dir, exist_ok=True)

    def make_inputs(B, E, D, H, N, require_grad=False):

        w = torch.randn(
            B, E, H, D, device="cuda", dtype=torch.bfloat16, requires_grad=require_grad
        )

        x = torch.randn(
            B, D, N, device="cuda", dtype=torch.bfloat16, requires_grad=require_grad
        )
        mean_size = N // E
        ...
        return w, x, group_sizes

    B, E, D, H, N = 4, 8, 512, 512, 4096
    inputs = make_inputs(B, E, D, H, N)

    name_2_fn = {
        # "triton_group_gemm_compile": torch.compile(
        #     grouped_gemm_transpose_triton  #  , mode="reduce-overhead"
        # ),
        "triton_group_gemm": grouped_gemm_transpose_triton,
    }
    B_list = [4, 8, 16]
    B_list = [4]
    # B_list = [2]
    L_list = [4096, 8192, 16384, 32768]
    ...
    for B in B_list:
        for D in D_list:
            for H_ratio in H_ratio_list:
                ...
                for L in L_list:
                    print(f"Running B={B}, L={L}, D={D}, H={H}, E={E}")
                    inputs = make_inputs(B, E, D, H, L, require_grad=True)
                    FLOPS = 2 * L * D * H * B
                    single_name_to_stats_dict = run_benchmark_fwd_only(
                        name_2_fn,
                        inputs,
                        repeats=10,
                        warmup=10,
