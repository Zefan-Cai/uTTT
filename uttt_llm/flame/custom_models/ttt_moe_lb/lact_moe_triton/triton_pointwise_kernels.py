...
def triton_swiglu_bwd_bwd_fused_cat_inp_out(
    ...
    # Cast to match your reference function’s return convention
    # incase input lr are fp16, cast the grad_lr* to fp16
    return (
        grad_dh,
        grad_x0_x2,
...
def test_speed():
    ...
    B = 4
    D = 768
    L_list = [1024, 2048, 4096, 8192, 16384]
    lr_dtype = torch.float32

    name_2_fn = {
        "torch_compile": torch.compile(ref_pytorch_swiglu_bwd_bwd_fused_cat_inp_out),
        "triton_fused": triton_swiglu_bwd_bwd_fused_cat_inp_out,
    }
    for L in L_list:
        inps = make_inputs(B, D, L, lr_dtype)
        FLOPS = B * D * L * 10
        print(f"Running B={B}, L={L}, D={D}")
