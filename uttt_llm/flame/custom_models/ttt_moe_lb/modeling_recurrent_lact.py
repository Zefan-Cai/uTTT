...
from .configuration_recurrent_lact import RecurrentLactRefMoeV5Config
...
class MoeModelOutput(BaseModelOutputWithPast):
    ...
    moe_stats: Optional[dict[str, torch.Tensor]] = None
...
_MOE_V5_EXPERT_CHUNK_SIZE = max(
    1,
    int(
        os.environ.get(
            "LACT_MOE_V5_EXPERT_CHUNK_SIZE",
            os.environ.get(
...
_MOE_V5_UPDATE_GRAD_DTYPE = os.environ.get(
    "LACT_MOE_V5_UPDATE_GRAD_DTYPE",
    os.environ.get(
...
def _moe_v5_update_grad_dtype() -> torch.dtype:
    if _MOE_V5_UPDATE_GRAD_DTYPE in {"bf16", "bfloat16"}:
        return torch.bfloat16
    if _MOE_V5_UPDATE_GRAD_DTYPE in {"fp32", "float32"}:
        return torch.float32
    raise ValueError(
        "LACT_MOE_V5_UPDATE_GRAD_DTYPE must be 'bf16' or 'fp32', "
        f"got {_MOE_V5_UPDATE_GRAD_DTYPE!r}",
    )
...
class FastWeightMLPSubNNMoeV5(base.FastWeightMLPSubNN):
    """Per-layer MoE fast-weight memory using the grouped flash FFN path."""

    def __init__(self, config: RecurrentLactRefMoeV5Config, layer_idx: int):
    ...
    def _route_tokens(
        ...
        with torch.autocast(device_type=x.device.type, enabled=False):
            ...
            if router_bias is not None:
                logits = logits + router_bias.to(device=logits.device, dtype=logits.dtype)
    ...
    def _apply_moe_fast_weights_slow(
        ...
        expert_mask, _, router_probs, lb_info = self._route_tokens(
            x,
            router_proj_weights,
            router_bias,
        )
    ...
    def _apply_moe_fast_weights_grouped(
        ...
        if (
            x.device.type != "cuda"
            or _permute_with_expert_mask is None
            or _unpermute_and_merge_with_probs is None
            or _grouped_swiglu_ffn_fwd is None
        ):
            return self._apply_moe_fast_weights_slow(
                x,
                w0,
                w1,
                w2,
        ...
        expert_mask, group_sizes_raw, router_probs, lb_info = self._route_tokens(
            x,
            router_proj_weights,
            router_bias,
        )
    ...
    def forward(
        ...
        out, lb_info = self._apply_moe_fast_weights(
            q,
            w0,
            w1,
            w2,
    ...
    def _apply_update(
        ...
        def _apply_one_weight(
            ...
            chunk_size = _MOE_V5_EXPERT_CHUNK_SIZE
    ...
    def update(
        ...
        grads, m_coeff, update_lb_info = self.compute_raw_grad(
            ...
            grad_dtype=_moe_v5_update_grad_dtype(),
        )
...
class RecurrentLactRefMoeV5Block(base.RecurrentLactBlock):
    def __init__(self, config: RecurrentLactRefMoeV5Config, layer_idx: int):
        super().__init__(config, layer_idx)
        if config.use_memory_block:
            self.memory = FastWeightMLPSubNNMoeV5(config, layer_idx)
...
class RecurrentLactRefMoeV5Model(base.RecurrentLactRefModel):
    config_class = RecurrentLactRefMoeV5Config
    _no_split_modules = ["RecurrentLactRefMoeV5Block"]

    def __init__(self, config: RecurrentLactRefMoeV5Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [
                RecurrentLactRefMoeV5Block(config, layer_idx)
    ...
    def _maybe_update_loss_free_bias(self) -> None:
        ...
        with torch.no_grad():
            ...
            if self.memory_lb_scope == "pool":
                ...
                target = counts.mean(dim=-1, keepdim=True)
                bias.add_(self.memory_loss_free_update_rate * torch.sign(target - counts))
    ...
    def _compose_memory_update_inputs(
        self,
        layer: RecurrentLactRefMoeV5Block,
        layer_idx: int,
    ...
    def _update_generation_memory_blocks(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            fastw, masterw, momentum_buf, update_lb_info = layer.update_fast_weight(
                ...
                ki=ki,
                vi=vi,
                router_bias=self._get_router_bias(layer_idx),
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if output_attentions:
            warnings.warn(
                "`RecurrentLactRefMoeV5Model` does not support output attention weights, "
        ...
        if not self.use_memory_block:
            raise ValueError("ttt_moe_lb requires use_memory_block=True")
    ...
    def extra_repr(self) -> str:
        ...
        lines = [
            parent_repr,
            "memory_pool_mode: per_layer_moe_v5",
...
def _init_moe_v5_weights(
    self,
    module: nn.Module,
    rescale_prenorm_residual: bool = False,
    num_residuals_per_layer: int = 2,
):
    if isinstance(module, FastWeightMLPSubNNMoeV5):
        module.reset_parameters()
        return
    if isinstance(module, RecurrentLactRefMoeV5Model):
        return
...
RecurrentLactRefMoeV5Model._init_weights = _init_moe_v5_weights


class RecurrentLactRefMoeV5ForCausalLM(base.RecurrentLactRefForCausalLM):
    config_class = RecurrentLactRefMoeV5Config
    _no_split_modules = ["RecurrentLactRefMoeV5Block"]
    _init_weights = _init_moe_v5_weights

    def __init__(self, config: RecurrentLactRefMoeV5Config):
        base.RecurrentLactRefPreTrainedModel.__init__(self, config)
        self.model = RecurrentLactRefMoeV5Model(config)
...
RecurrentLactRefModel = RecurrentLactRefMoeV5Model
RecurrentLactRefForCausalLM = RecurrentLactRefMoeV5ForCausalLM
