        ...
        **kwargs: Unpack[Any],
        ...
        if use_cache:
            if is_recurrent_lact_cache(past_key_values):
                generation_cache = past_key_values
                attention_cache = generation_cache.get("attention_cache")
                if attention_cache is None:
                    attention_cache = Cache()
                    generation_cache["attention_cache"] = attention_cache
                initial_seen_tokens = recurrent_lact_cache_seen_tokens(generation_cache)
            else:
                attention_cache = (
                    past_key_values
                    if isinstance(past_key_values, Cache)
                    else Cache.from_legacy_cache(past_key_values)
                )
                initial_seen_tokens = recurrent_lact_cache_seen_tokens(attention_cache)
                generation_cache = make_recurrent_lact_cache(
                    layers=[{} for _ in self.layers],
                    seen_tokens=initial_seen_tokens,
                    pending_start=initial_seen_tokens,
                    batch_size=hidden_states.size(0),
                    attention_cache=attention_cache,
                )
            generation_cache["batch_size"] = hidden_states.size(0)
        else:
        ...
        if use_cache and generation_cache is not None:
            generation_cache["seen_tokens"] = initial_seen_tokens + inputs_embeds.size(1)
            generation_cache["batch_size"] = inputs_embeds.size(0)
            next_cache = generation_cache
