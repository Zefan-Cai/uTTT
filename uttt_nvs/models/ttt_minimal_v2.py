...
# The passing argument ttt_config is a list of TTTConfig named tuple.
# We provided some of the example ttt_config below.
TTTConfig = collections.namedtuple("TTTConfig", ["start", "end", "fast_weight", "update", "apply"])

def parallel_ttt_config(update_minibatch=1024, apply_only_minibatch=1024, length=10240, update_length=None):
    ...
    for start in range(0, update_length, update_minibatch + apply_only_minibatch):
        config.append(TTTConfig(
            start=start, end=start + update_minibatch,
            fast_weight=True, update=True, apply=True
        ))
        if apply_only_minibatch > 0:
            config.append(TTTConfig(
                start=start + update_minibatch, end=start + update_minibatch + apply_only_minibatch,
                fast_weight=False, update=False, apply=True
            ))

    if update_length < length:
        config.append(TTTConfig(start=update_length, end=length, fast_weight=False, update=False, apply=True))
    return config


def mini_update_config(update_minibatch=1024, length=10240, update_length=None):
    """
    ...
    config.append(TTTConfig(start=0, end=length, fast_weight=False, update=False, apply=True))
    return config


def compression_ttt_config(minibatch=1024, length=10240):
    config = []
    for start in range(0, length, minibatch):
        config.append(TTTConfig(start=start, end=start + minibatch, fast_weight=True, update=True, apply=True))
    return config


def apply_ttt_config(minibatch=1024, length=10240):
    config = []
    for start in range(0, length, minibatch):
        config.append(TTTConfig(start=start, end=start + minibatch, fast_weight=False, update=False, apply=True))
    return config
...
def fast_weight_swish_glu_weight_norm_mini_batch_apply(
    ...
    output = []
    for op_idx, (start, end, fast_weight, update, apply) in enumerate(ttt_config):

        w0_now, w1_now, w2_now = w0, w1, w2

        # print(f"Start: {start}, End: {end}, Fast Weight: {fast_weight}, Update: {update}")
...
class FastWeightGluMLPMultihead(nn.Module):
    ...
    def __init__(
        ...
        head_dim: int,
        inter_multi: int = 1,
        expansion_multi: int = 0,
        bias: bool = False,
        use_o_norm=True,
        base_lr=0.01,
        lr_parameterization="mamba",  # study later
        muon_update_steps=0,
        weight_norm=True,
        vinosilu=False,
    ):
        ...
        elif lr_parameterization.lower() == "nerf":
            self.base_lr_inv = inv_softplus(-math.log(base_lr))
            print(self.base_lr_inv)
        elif lr_parameterization.lower() == "sigmoid":
            self.base_lr_inv = inv_sigmoid(base_lr)
            print(self.base_lr_inv)
        else:
            raise ValueError(f"Unknown lr_parameterization: {lr_parameterization}")

        self.use_o_norm = use_o_norm
        if self.use_o_norm:
            self.o_norm = torch.nn.RMSNorm(
    ...
    def forward(self, x: torch.Tensor, vis_dict=None, shape_info=None, *args):
        ...
        elif self.lr_parameterization == "nerf":
            positive_term = torch.nn.functional.softplus(lr.float() + self.base_lr_inv)
            lr = torch.exp(-positive_term)
        elif self.lr_parameterization == "sigmoid":
            lr = torch.sigmoid(lr + self.base_lr_inv)
        else:
            raise ValueError(f"Unknown lr_parameterization: {self.lr_parameterization}")
        ...
        if shape_info.get("return_state", False):
            return output, {"w0": w0, "w1": w1, "w2": w2}
        else:
            return output, {}

    def non_param_flops(self, l):
        # l is seq length
        fast_weight_mlp_flops = 14 * l * self.dim * self.dim * self.dim * self.inter_multi
        toqkv_flops = 6 * l * self.dim * self.dim
        c_proj_flops = 2 * l * self.dim * self.dim
        return fast_weight_mlp_flops + toqkv_flops + c_proj_flops

    def extra_repr(self) -> str:
        return (f"w0 shape: {self.w0.shape}, w1 shape: {self.w1.shape}, w2 shape: {self.w2.shape}, "
                f"Muon update steps: {self.muon_update_steps}, lr parameterization: {self.lr_parameterization}."
                f"Base lr: {self.base_lr}, "
                f"weight norm: {self.weight_norm}, ")
